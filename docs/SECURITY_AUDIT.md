...
| The App Store Connect private key could be inherited by build, plugin, test, and scan subprocesses | High | The release entrypoint captures the key into a non-exported shell variable, unsets the exported value before project code runs, and writes a mode-`0600` temporary key only immediately before `notarytool` | ShellCheck, secret scans, fail-fast signing probe, and clean final autoreview |
...
| Apple notarization could wait indefinitely when a submission remained in progress | Low | The foreground `notarytool` wait is bounded to 30 minutes and leaves timed-out submissions available for reconciliation | ShellCheck and final autoreview |
...
6. **Secrets:** Gitleaks and TruffleHog found no verified or unverified secrets. The repository contains no credential fixtures or environment files. The release process removes the exported notarization private key before any project subprocess and materializes it only in a mode-`0600` temporary file for `notarytool`.
